...
function updateEmpathyRankings() {
    ...
    if (sortedDeclarations.length >= 1) {
        firstRankElement.textContent = `선언문 ${sortedDeclarations[0].id} (${sortedDeclarations[0].empathyCount}표)`;
    } else {
        firstRankElement.textContent = '-';
    }
    
    if (sortedDeclarations.length >= 2) {
        secondRankElement.textContent = `선언문 ${sortedDeclarations[1].id} (${sortedDeclarations[1].empathyCount}표)`;
    } else {
        secondRankElement.textContent = '-';
    }
    
    if (sortedDeclarations.length >= 3) {
        thirdRankElement.textContent = `선언문 ${sortedDeclarations[2].id} (${sortedDeclarations[2].empathyCount}표)`;
    } else {
...
function updateEmpathyResults() {
    ...
    if (sortedDeclarations.length === 0) {
        ...
        noDataMessage.textContent = '아직 공감 표시가 없습니다.';
    ...
    sortedDeclarations.forEach((declaration, index) => {
        ...
        title.textContent = `${rankIcon}선언문 ${declaration.id}`;
